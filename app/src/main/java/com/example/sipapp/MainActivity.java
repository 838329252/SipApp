...
public class MainActivity extends AppCompatActivity {
    ...
    public void login(View view) {
        ...
        String realm=mEtRealm.getText().toString().trim();
        String port = mEtPort.getText().toString().trim();

        if (TextUtils.isEmpty(server) || TextUtils.isEmpty(account)
                || TextUtils.isEmpty(pwd) || TextUtils.isEmpty(port)||TextUtils.isEmpty(realm)) {
        ...
        mAccount.setRealm(realm); //realm指的是：sip:1004@192.168.2.243中的192.168.2.243
